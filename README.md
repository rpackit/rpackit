...
`WebSocket()` calls. A Tauri request interceptor or native loopback proxy must
keep the credential outside JavaScript, add it to every request for the exact
`http://127.0.0.1:<port>` origin, and strip it before any external redirect.
The threat model excludes malicious same-user processes, administrator or
debugger access, and untrusted app/package code running inside the credential-
bearing R process. Tauri project generation, native executable packaging,
static-web builders, and server builders remain later milestones.
