...
The lifecycle manager starts the bundled `Rscript`, waits for both a versioned
launcher event and a real HTTP response, and requests graceful shutdown through
a private control file. If graceful shutdown times out, it asks `processx` to
terminate the tracked process and its known tree, with a tracked-process kill
as fallback.
